...
{
	QPainterPath pathFirstPlan = buildPath(QSizeF(size.width(), size.height() / 6));
...
QPixmap BackgroundBuilder::buildBackground(QSizeF const& size)
{
	...
	QPixmap backgroundPixmap(size.toSize() + QSize(0, treeSize.height()));
	backgroundPixmap.fill(Qt::transparent);

	QPainter painter(&backgroundPixmap);
	painter.translate(0, treeSize.height());
	painter.setRenderHint(QPainter::Antialiasing);
	painter.setBrush(QBrush(0x77b643));

	paintDecoration(painter, path, 1.0, 10);
	...
	paintDecoration(painter, path, 1.0, 15);
...
void BackgroundBuilder::paintDecoration(QPainter& painter, QPainterPath const& path, double offsetJitter, quint32 density)
{
	QList<QPair<QRect, int> > decorations;
	for (double delta = 0; delta < 100; delta += QRandomGenerator::global()->bounded(quint32(1), density))
	{
		int const jitter = QRandomGenerator::global()->bounded(int(offsetJitter * treeSize.height()), int(treeSize.height()));
